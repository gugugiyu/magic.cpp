...
/**
 * Fetch wrapper with automatic timeout handling.
 * Creates an AbortController with timeout if no signal is provided,
 * or combines the provided signal with a timeout.
 *
 * @param url - Fetch URL
 * @param options - Fetch options (signal will be overridden if timeout is set)
...
export async function fetchWithTimeout(
	...
	const { timeoutMs: optionTimeout, ...fetchOptions } = options ?? {};
	const effectiveTimeout = optionTimeout ?? timeoutMs;

	const controller = createLinkedController(options?.signal ?? undefined);
