...
<div class="flex flex-col gap-2 {className}">
				...
				{#each argumentEntries as [key, value] (key)}
					<Tooltip.Root>
						<Tooltip.Trigger>
							...
							<span
								class="rounded-sm bg-purple-bg px-1.5 py-0.5 text-[10px] leading-none text-purple transition-opacity {hoveredArgKey &&
								hoveredArgKey !== key
									? 'opacity-30'
									: ''}"
	...
	{:else if isLoading}
		<Card
			class="relative overflow-hidden rounded-[1.125rem] border border-purple bg-purple/10 px-1 py-2 backdrop-blur-md dark:bg-purple/20"
		>
			<div
				class="overflow-y-auto {paddingClass}"
				style="{maxHeightStyle} overflow-wrap: anywhere; word-break: break-word;"
	...
	{:else if hasContent}
		<Card
			class="relative overflow-hidden rounded-[1.125rem] border border-purple bg-purple/10 py-0 text-foreground backdrop-blur-md dark:bg-purple/20"
		>
			<div
				class="overflow-y-auto {paddingClass}"
				style="{maxHeightStyle} overflow-wrap: anywhere; word-break: break-word;"
			>
				<span class="{textSizeClass} whitespace-pre-wrap">
					...
					{#each contentParts as part, i (i)}{#if part.argKey}<span
								class="rounded-sm bg-purple-bg/70 px-0.5 text-purple transition-opacity {hoveredArgKey &&
